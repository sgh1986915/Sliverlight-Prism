...
using Procbel.Apps.Silverlight.MainRepository.Extensions;
using Procbel.Apps.Silverlight.Converters;

namespace Procbel.Apps.Silverlight.Helpers
{
    public class CustomAppointmentMain : Appointment
    {
        private Activity activity;

        public CustomAppointmentMain()
        {
            this.Activity = new Activity() { Status = 0, Type = 0, Priority = 1 };
            this.UniqueId = "0";
        }

        public CustomAppointmentMain(Activity activity)
        {
        ...
        public Activity Activity
        {
            get
            {
                return this.Storage<CustomAppointmentMain>().activity;
            }
            set
            {
                using (var storage = this.Storage<CustomAppointmentMain>())
                {
        ...
        {
            base.CopyFrom(other);
            CustomAppointmentMain appointment = other as CustomAppointmentMain;
            if (appointment != null)
            {
        ...
        {
            IAppointment appointment = new CustomAppointmentMain();
